...
<template>
  <div>
    ...
    <el-row style="margin-top:20px" type="flex">
      ...
      <el-col>
        <z-button @click="tnsView" type="text" icon="el-icon-view"
        >显示浏览效果
        </z-button
        >
      </el-col>
    ...
    <el-row>
      <z-upload ref="uploadFiles" :isDrag="false" :limit="5" :show-file-list="false" :accept="['*']"
                @on-upload-success="onUploadProcessFileSuccess"></z-upload>
    </el-row>
    <el-row>
    </el-row>
    <el-row>
      <z-table
        :columns="columns" :data="tableData" ref="tableRef" :show-checkbox="false" :border="true"
        :fit="true"
      >
        <template slot="phone">
          <el-table-column label="电话" header-align="center">
            <el-table-column label="座机" prop="xxx"></el-table-column>
            <el-table-column label="手机" prop="xxx"></el-table-column>
          </el-table-column>
        </template>
        <template slot="age">
          <span>123</span>
...
import ZTooltip from '../components/z-tooltip.vue'
import ZDate from '../components/z-date-picker.vue'
import ZUpload from '../components/z-upload/z-upload'
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          isBorder: true,
          tagColor: (row) => {
            return { 0: '#000', 1: '#fff' }[row.status]
          },
          text: (row) => {
            return { 0: '关闭', 1: '开始' }[row.status]
          },
          isFilter: true,
          filters: [
            { text: '关闭', value: 0 },
      ...
      tableData: [
        {
          ...
          status: 1,
          photo: 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fup.enterdesk.com%2Fedpic%2Fc8%2Fe3%2Fd9%2Fc8e3d9870576652b3204a01e69cfabed.jpg&refer=http%3A%2F%2Fup.enterdesk.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1645002259&t=2c40ae953a522e4d480bf9af288fa558'
        },
        { xxx: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', status: 0 },
        { xxx: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', status: 1 }
      ]
    }
  },
  methods: {
    ...
    tnsView () {
      ...
      utils.previewImages({
        ...
        imageList: [
          {
            larger: 'https://ss0.baidu.com/7Po3dSag_xI4khGko9WTAnF6hhy/zhidao/pic/item/9c16fdfaaf51f3dee3fbb43696eef01f3a297912.jpg',
    ...
    onUploadProcessFileSuccess (files) {
